...
class ConfirmationPage(BasePage):
    def __init__(self, controller):
        ...
        tk.Label(self, text="Ensure the chamber is free of samples and only exposed to clean air.",
                 fg="white", bg="#2c2c2c", font=("Arial", 14), wraplength=400).pack(pady=20)

        tk.Button(self, text="Confirm and Start Gathering", bg="green", fg="white", font=("Arial", 14),
...
# BASELINE DATA GATHERING FOR APP TESTING
class ConfirmationPage(BasePage):
    def __init__(self, controller):
        super().__init__(controller, controller)
    ...
    def collect_baseline_data(self):
        ...
        # Open the CSV file to simulate data collection
        with open(filename, 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(["MQ2", "MQ3", "MQ4", "MQ5", "MQ6", "MQ7", "MQ8", "MQ9", "MQ135", "MQ137"])
            ...
            try:
                while time.time() - start_time < duration:
                    # Generate random sensor values between 100 and 800
                    data = [random.randint(100, 800) for _ in range(10)]
                    csvwriter.writerow(data)
                    # Notify ProgressPage to update progress
                    elapsed_time = int(time.time() - start_time)
                    self.controller.pages["ProgressPage"].update_progress(elapsed_time)
...
class ProgressPage(BasePage):
    ...
    def update_progress(self, elapsed_time):
        ...
        self.label.config(text=f"Gathering Baseline Data: {elapsed_time} seconds / 60 seconds")
...
class SamplePrepPage(BasePage):
    ...
    def update_progress(self):
        ...
        if self.elapsed_time < 61:
            ...
            self.progress.update()
            self.label.config(text=f"Elapsed Time: {self.elapsed_time} seconds / 60 seconds")

            # Increment time and schedule next update
            self.elapsed_time += 1
...
class SampleConfirmationPage(BasePage):
    ...
    def collect_sample_data(self):
        ...
        # Open the CSV file for sample data collection
        with open(filename, 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(["MQ2", "MQ3", "MQ4", "MQ5", "MQ6", "MQ7", "MQ8", "MQ9", "MQ135", "MQ137"])
            ...
            try:
                while time.time() - start_time < duration:
                    ...
                    # Update progress on the progress page
                    elapsed_time = int(time.time() - start_time)
